...
//density of a fermi gas and mapped to a bound of (0,1)
    ...
    (std::pow((1.0 - t)/t,2.0) * 
    get_BoseEinstein_dist(params.temperature_, energy_epsilon, chem_potent_mu))) 
    / std::pow(t,2.0);
}
...
//integrand goes to 0 as t goes to 0.
        ...
        if(t_lower != 0.0) {
            ...
            density_lower = 
            get_density_integrand_bose(t_lower,
            ...
            if (std::isnan(density_lower)) {
                std::cout << "lower density is NAN in get_density_integral_bose!" << std::endl;
                density_lower = 0.0;
            }
        }
        ...
        if(t_upper != 1.0) {
            ...
            density_upper = 
            get_density_integrand_bose(t_upper, energy_epsilon_upper, 
                chem_potent, *par);
            ...
            if (std::isnan(density_upper)) {
                std::cout << "upper density is NAN in get_density_integral_bose!" << std::endl;
                density_upper = 0.0;
            }
        }
