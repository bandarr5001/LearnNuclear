...
int conditions(const gsl_vector* x, void* p,
    ...
    double condition2 = (energy_epsilon_Fermi_satdense/nSat_in_MeV3)
        - (energy_density_FG_satdense/(nSat_in_MeV3*nSat_in_MeV3)) + ((A/2.0)*(1/nSat_in_MeV3)) + B*((tau - 1.0)/tau)*(1/nSat_in_MeV3);

    gsl_vector_set(f, 1, condition2);

    //Calculated using Mathematica to brute force the deriivative of P_FG
...
double get_single_particle_potential(potential_results parameter_results,
    ...
    //std::cout << "parameter_results in potential function, A: " << parameter_results.A << " B: " << parameter_results.B << " tau: " << parameter_results.tau << std::endl;
 
    long double potential = parameter_results.A*(pparams.density_/nSat_in_MeV3) +
     parameter_results.B*
